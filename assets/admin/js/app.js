$(function() {
  ...
  if ($('#navigation').length > 0) {
    ...
    $('#navigation > dl > dt, #navigation > dl > dd > div > ul > li > a').each(function(i) {
      $(this).focus(function() {
        if (this.blur)
          $(this).blur();//去掉虚线框
      });
...
var Navigation = {
  ...
  getPage: function(rel) {
    ...
    if (_obj.length == 0) {
      ...
      if (document.documentElement.clientHeight < document.documentElement.scrollHeight) {
        ...
        if (_dcw.length > 0)
          ...
          Navigation.getSearchData(_dgwObj);
        _dgwObj = _dcw = null;
      }
      _id = _container = _obj = null;
      window.location.hash = rel;
    ...
    var _menu = $('#menu_' + _aryRel[0]);
    $('> dd > div > ul > li > a[rel="' + rel + '"]', _menu).parent().addClass('slt');
    _menu.addClass('slt').siblings().removeClass('slt');
  },
  getSearchData: function(t) {
    $('#main_loading').fadeIn(function() {
      $(t).dataGridOptions({
        params: $('form[name=search_from]', t).serializeArray()
      }).dataGridReload();
      //$('#main_loading').fadeOut();
    });
  },
  flushContentHeight: function(t) {//刷新内容高度
    ...
    if (_$ie6)//ie6
      _offset.top -= 80;
    t.height(_$top.height() - _offset.top - 12);
    ...
    if (_total > 0) {
      $.dialog.locking.confirm('prompt', aryStr[0] + ' ' + _total + ' ' + aryStr[1], function() {
        ...
        for (var i = 0; i < _total; i++) {
          if (i > 0)
            _id += ',';
          _id += _obj.eq(i).attr('id').substring(3);
        }
        var _postObj = {};
        if (typeof action == 'object') {
          _postObj = action;
