...
/**
 * _path 获取组件核心文件pxdialog.js所在的绝对路径
 * _args 获取pxdialog.js文件后的url参数组，如：pxdialog.js?self=true&skin=aero中的?后面的内容
 */
_args, _path = (function( script, i, me )
{
	...
	for( ; i < l; i++ )
	{
		...
		if( me.substr(me.lastIndexOf('/')).indexOf('pxdialog') !== -1 )
		    break;
	}
 ...
 * @param  {String}
 * @return {String||null}
 * @demo pxdialog.js?skin=aero | _getArgs('skin') => 'aero'
 */
...
/** 获取 pxdialog 可跨级调用的最高层的 window 对象和 document 对象 */
...
_$ie6 = _ie6, _$doc = $(_doc), _$top = $(_top), _$html = $(_doc.getElementsByTagName('html')[0]);
...
(function(style){
	if(!style)
	{
	    ...
	    link.rel = 'stylesheet';
		link.id = 'pxdialoglink';
		head.insertBefore(link, head.firstChild);
	}
})(_doc.getElementById('pxdialoglink'));
...
var pxdialog = function( config )
{
	config = config || {};

	var api, setting = pxdialog.setting;
	...
	api = pxdialog.list[config.id];
	//未设置 resetConfig 则直接返回对象
	if(!config.resetConfig && api) return api.zindex().focus();
	...
	//设置了 resetConfig 则重设窗口
	if(config.resetConfig && api) return api.resetConfig(config);

	// zIndex全局配置
	pxdialog.setting.zIndex = config.zIndex;

	_count++;

	return pxdialog.list[config.id] = _singleton ?
	    _singleton._init(config) : new pxdialog.fn._init( config );
};

pxdialog.fn = pxdialog.prototype =
{
    constructor: pxdialog,
	...
	resetConfig: function( config, keep )
	{
		...
		//重设值不改变zIndex, lock
		config.zIndex = that.config.zIndex;
		config.lock = that.config.lock;
		// 合并默认配置
		for( var i in pxdialog.setting )
		{
			if( config[i] === undefined ) config[i] = pxdialog.setting[i];
		}

		if (keep === true)
		{
			for( var i in config )
				that.config[i] = config[i];
		}
		else {
			that.config = config;
		}
		...
		config.button && that.button.apply( that, config.button );

		that.title( config.title );
		that.content( content, true, isIfr );
	...
	button: function()
	{
		...
		for( ; i < ags.length; i++ )
		{
			...
			if( isNewButton )
			{
				...
				if (item.unshift && buttons.firstChild)
				{
	...
	content: function( msg, add, frm )
	{
		...
		var that = this, DOM = that.DOM,
			...
			$content = DOM.content,
			loading = pxdialog.setting.content;
		...
		if( frm )
		{
			...
			{
				that._ajaxload( msg.split('load:')[1] );
			}
			else
	...
	show: function( args )
	{
		...
		if( !args && this._lock )
		    $('#px_lockmask',_doc)[0].style.display = '';
	...
	hide: function( args )
	{
		...
		if( !args && this._lock )
		    $('#px_lockmask',_doc)[0].style.display = 'none';
	...
	{
		var that = this, frm,
		    index = pxdialog.setting.zIndex - 1,
			//config = that.config,
			mask = $('#px_lockmask',_doc)[0],
	...
	unlock: function()
	{
		...
		{
			// 无限级锁屏，针对iframe
			if( that.config.parent && that.config.parent._lock )
			{
//			    index = that.config.parent.DOM.wrap[0].style.zIndex;
//				mask.style.zIndex = parseInt(index,10) - 1;
				mask.style.zIndex = that.config.parent.config.zIndex;
				that.config.parent.zindex();
			}
			else
			{
				//不需要传parent锁屏
				list = $.map(list, function (e) {
					if (e.config.lock) {
						return e.config.zIndex;
					}
				});
				if (list.length > 1)
				{
					//ie 7 8 下必须指定list长度，否则无法删除
					list.splice($.inArray(mask.style.zIndex, list), 1);
					mask.style.zIndex = Math.max.apply(null, list);
					list = pxdialog.list;
					for (var i in list)
					{
						if (list[i].config.zIndex == mask.style.zIndex)
						{
							list[i].zindex(); break;
						}
					}
	...
	close: function()
	{
		...
		if( pxdialog.focus === that ) pxdialog.focus = null;
		...
		else
		{
		    ...
		    DOM.content.html('');
			if (DOM.content[0].style.height != 'auto')
				DOM.content[0].style.height = 'auto';
	...
	get: function( id, object )
	{
		...
		{
			if( object === true )
			    return pxdialog.list[id];
			else
			    return pxdialog.list[id].content || null;
		}
	...
	wrapForm: function( ) {
		var DOM = this.DOM,
		formName = 'post_form', formId = '';
//		formName = formName || 'post_form';
//		if (typeof formId != 'undefined')
//			formId = ' id = "' + formId + '"';

		if (!this.DOM.form)
		{
			this.DOM.dialog.wrap('<form action="" name="'
				+ formName + '"' + formId
				+ ' method="post" onsubmit="return false;"></form>');
			this.DOM.form = this.DOM.dialog.parent();
			//this.DOM.form = true;
		}
	},
	...
	_iframe: function( url )
	{
		...
		var load = that._fmLoad = function()
		{
			...
			? $idoc.width() + (_ie6 ? 0 : parseInt($ibody.css('marginLeft'), 10))
			: config.width;
	...
	_ajaxload: function( url )
	{
		...
		$content.load(url, function(){
			$content.css('padding', '10px 0 0 0');
			//console.log($content.height() + ' ' + parseInt(_$top.height() * 0.7, 10));
			that._trimHeight();
			if( !that._maxState && config.lwidth )
			{
			    that.DOM.wrap[0].style.width = config.lwidth + 'px';
			    that.DOM.main[0].style.width = (config.lwidth - 12) + 'px';
//				config.width = config.lwidth;
	...
	_getDOM: function()
	{
        ...
        var name, i = 0,
			DOM = { wrap: $(wrap) },
			domTree = ['main', 'icon', 'border', 'rb', 'title'
						, 'max', 'max_b', 'min', 'min_b',  'res'
						, 'res_b', 'res_t', 'dialog', 'close', 'content'
						, 'buttons'],
		...
		for( ; i < len; i ++ )
		{
			...
			if(name && $.inArray(name, domTree) != -1) DOM[name] = $(els[i]);
	...
	_ie6PngFix: function()
	{
	    if( _ie6 )
		{
			var i = 0, elem, png, pngPath, runtimeStyle,
				path = pxdialog.setting.path + '/skins/',
	...
	_setFixed: function( el )
	{
		...
		if( _ie6 )
		{
			var sLeft = _$top.scrollLeft(),
				...
				left = parseInt(style.left, 10) - sLeft,
				top = parseInt(style.top, 10) - sTop,
	...
	_reset: function()
	{
		...
		if(test)
		{
			//IE6下遮罩大小改变
			if( that._lock && _ie6 )
			    $('#px_lockmask',_doc).css({ width:tw + 'px', height:tt + 17 + 'px' });
	...
	//height auto时防止返回值高度过大撑出屏幕
	_trimHeight: function()
	{
		if (this.DOM.content.height() > parseInt(_$top.height() * .9, 10))
			this.DOM.content[0].style.height = parseInt(_$top.height() * 9, 10) + 'px';
	}
};

pxdialog.fn._init.prototype = pxdialog.fn;
...
_top != window && $(window).bind('unload',function()
{
	...
	$('#px_lockmask',_doc)[0] && $('#px_lockmask',_doc).remove();
	$('#px_dragmask',_doc)[0] && $('#px_dragmask',_doc).remove();
});
...
pxdialog.setting =
{
	...
	max: true,                  // 是否显示最大化按钮
	min: true,                  // 是否显示最小化按钮
	zIndex: 1888,				// 对话框叠加高度值(重要：此值不能超过浏览器最大限制)
	resize: true,				// 是否允许用户调节尺寸
	drag: true, 				// 是否允许用户拖动位置
	cache: true,                // 是否缓存窗口内容页
	data: null,                 // 传递各种数据
	extendDrag: false           // 增加pxdialog拖拽体验
};
...
$(function(){
	...
	{
	    if(_count) return;
		pxdialog({left:'-9999em',time:9,fixed:false,lock:false,focus:false});
	},150);

	// 增强pxdialog拖拽体验（可选外置模块，如不需要可删除）
	// 防止鼠标落入iframe导致不流畅，对超大对话框拖动优化
	pxdialog.setting.extendDrag &&
	(function(dragEvent){
		...
		{
			var api = pxdialog.focus,
				main = api.DOM.main[0],
				iframe = api.iframe;

			dragEvent._start.apply(this, arguments);
			style.display = 'block';
			style.zIndex = pxdialog.setting.zIndex + 3;
...
$.fn.dialog = function()
{
	...
	this.bind('click',function(){ pxdialog.apply(this,config); return false; });
	return this;
};

window.pxdialog = $.dialog = pxdialog;

})( this.jQuery || this.pxcore, this );
 ...
 *------------------------------------------------
 */
;(function( $, pxdialog, undefined ){

var _zIndex = function()
{
    return pxdialog.setting.zIndex;
};
 ...
 * @param	{String}	消息内容
 */
pxdialog.alert = function( content, callback, icon, parent )
{
	return pxdialog({
		title: '系统提示',
		id: 'Alert',
		...
		fixed: true,
		lock: true,
		content: '<span class="alert' + (icon ? ' ' + icon : '') + '">' + content + '</span>',
		ok: true,
		resize: false,
		close: callback,
		width: 300,
 ...
 */
pxdialog.confirm = function( content, yes, no, parent )
{
	return pxdialog({
		title: '系统提示',
		id: 'Confirm',
 ...
 * @param	{String}	默认值
 */
pxdialog.prompt = function( content, yes, value, parent )
{
	value = value || '';
	var input;
 ...
 */
pxdialog.tips = function( content, time, icon, callback )
{
 ...
 * @param   {Function}  提示关闭时执行的回调函数
 */
pxdialog.locking = function( content, icon, callback )
{
	return pxdialog({
		resetConfig : true,
		title : false,
		...
		lock : true,
		content : content,
		//content: '<span class="alert">' + content + '</span>',
		resize : false,
		close : callback
	});
};
pxdialog.locking.alert = function( icon, content, callback )
{
	return pxdialog({
		resetConfig : true,
		id : 'Locking',
...
pxdialog.locking.confirm = function( icon, content, yes, no, init )
{
	return pxdialog({
		resetConfig : true,
		id : 'Locking',
...
pxdialog.locking.pager = function( config )
{
	config.resetConfig = true;
	config.id = 'Locking';
	config.zIndex = _zIndex();
	return pxdialog(config);
};
pxdialog.locking.self = function()
{
	return pxdialog.list['Locking'];
};
pxdialog.locking.remove = function()
{
	if (pxdialog.list['Locking'])
		pxdialog.list['Locking'].close();
};
pxdialog.imgLoader = function(domChunk)
{
	$(domChunk).click(function(){
		pxdialog.imgLoader.show(this.title || null, this.href);
		this.blur();
		return false;
	});
};
pxdialog.imgLoader.show = function(title, src)
{
	pxdialog({
		resetConfig : true
		, title : false
	...
	imgPreloader.onload = function(){
		...
		pxdialog({
			...
			, lock : true
			, min : false
			, content : '<img src="' + imgPreloader.src + '" width="'
					+ imgPreloader.width + '" height="' + imgPreloader.height + '" />'
			, padding : 0
			, init : function() {
				$('#px_lockmask').unbind('click').click(function(){
					pxdialog.list['ImgLoader'].close();
				});
			}
			, close : function() {
				$('#px_lockmask').unbind('click');
			}
		});
	};
...
})( this.jQuery||this.pxcore, this.pxdialog );