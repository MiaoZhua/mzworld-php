		...
		if (typeof PageAction[PageAction.currenPageActionClassName].doContentAction == 'undefined') {
			PageAction[PageAction.currenPageActionClassName].doContentAction = function() {
				var _addUser = function() {
					...
					if ($.trim(_userPwd.val()).length > 0
						&& ($.trim(_userPwd.val()).length < 5
						|| $.trim(_userPwd.val()).length > 12)) {
						$.dialog.alert('请输入5至12位密码', function(){
							_userPwd.focus();
						});
						return false;
					}
					if (PageAction.currenPageAction == 'Add') {
...
<table width="100%" cellpadding="0" cellspacing="0" class="data_input">
	<tr>
		<th width="20%">语言</th>
		<td width="30%"><?= \Tools\Html::radio($this->pageControl, 'ilanguage', $this->rs, $this->__LANGUAGE_CONFIG__, $this->__LANGUAGE_ID__) ?></td>
		<th width="20%">修改时间</th>
		<td><?= \Tools\Html::setDate($this->pageControl, 'release_date', \Tools\Auxi::getTime(time()), '90%') ?></td>
	</tr>
	<tr>
		<th>添加时间</th>
		<td><?= \Tools\Html::setDate($this->pageControl, 'add_date', \Tools\Auxi::getTime(time()), '90%') ?></td>
		<th>所属角色</th>
		<td><?= $this->userRole ?>
		</td>
	</tr>
	<tr>
		<th>用 户 名</th>
		<td><?= \Tools\Html::text($this->pageControl, 'user_name', $this->rs) ?></td>
		<th>密码（不改勿填）</th>
		<td><input type="password" name="user_pwd" id="user_pwd" style="width:98%" value=""<?= $this->disabled ?>></td>
	</tr>
	<tr>
		<th>真实姓名</th>
		<td><?= \Tools\Html::text($this->pageControl, 'real_name', $this->rs) ?></td>
		<th>邮箱地址</th>
		<td><?= \Tools\Html::text($this->pageControl, 'email', $this->rs) ?></td>
	</tr>
