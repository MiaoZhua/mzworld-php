    ...
    $(function(){
        ...
        if (typeof PageAction[PageAction.currenPageActionClassName].doContentAction == 'undefined') {
            PageAction[PageAction.currenPageActionClassName].doContentAction = function() {
                var _test = Tools.compare('tutorial_chapter_id', 0, '请选择所属章节') &&
                    Tools.checkNull('question', '请输入问题');
                if (_test) {
                                    ...
                                    $.dialog.locking.confirm('success', '编辑成功，您还需要继续添加吗？', function(){
                                        ...
                                        Tools.addEditPageLoader()
                                            .resetConfig({
                                                ...
                                                + '&parentId=' + $('#tutorial_chapter_id').val()
                                            }, true);
...
<table width="100%" cellpadding="0" cellspacing="0" class="data_input">
    <tr>
        <th width="15%">所属章节</th>
        <td width="35%"><?= \Tools\Html::select($this->pageControl, 'tutorial_chapter_id', $this->rs ? $this->rs->tutorial_chapter_id : $_GET['parentId'], $this->aryTutorialChapter, '0') ?></td>
        <th width="15%">发布时间</th>
        <td><?= \Tools\Html::setDate($this->pageControl, 'release_date', \Tools\Auxi::getTime(time()), '90%') ?></td>
    </tr>
    <tr>
        <th>问题</th>
        <td colspan="3"><?= \Tools\Html::text($this->pageControl, 'question', $this->rs) ?></td>
    </tr>
    <tr>
        <th>封面图</th>
        <td colspan="3"><?= \Tools\Html::textarea($this->pageControl, 'answer', $this->rs, null, 5, null, ' onpropertychange="if(value.length>300) value=value.substr(0,300)"') ?></td>
    </tr>
</table>