...
/**
 * PageSupport
 * Controller.class.php, HttpHandler.class.php继承之后与拦截器共享数据
 * 目前只是在具体的业务类之前执行
...
//    protected function __Inject($db = null, $cache = null) {}
     ...
     * @param array $optBind select与数据库之间绑定的参数以及默认值 $isShowArticleList 是否只显示文章列表项
    ...
    protected function _selectIDTree($dbOpt = array('table' => '`#@__@table`','where' => '', 'bind' => null,
        'select' => '', 'order' => '', 'sort' => ''),
                                     $optBind = array('value' => 'fieldName', 'text' => 'fieldName',
                                         'selected' => '', 'isShowArticleList' => false),
                                     $selectControl = null,
        ...
        if (count($_rs) > 0) {
            foreach ($_rs as $m) {
                if ((isset($optBind['isShowArticleList']) && (intval($m->is_part) == 0 ||
                            intval($m->level) == 1)) ||
                    (isset($optBind['isShowArticleList']) && false === $optBind['isShowArticleList']) ||
                    !isset($optBind['isShowArticleList'])) {
     ...
     * @return type
     */
    protected function _getSort($dt = 'article_type', $field = 'sort') {
        //$this->db->debug = true;
    ...
    protected function _getArticleTags($id) {
        return implode(',', $this->db->mode(PDO::FETCH_COLUMN)->select('a.tags_text')
            ->table('`#@__@tags` a, `#@__@tags_list` b')
            ->where('b.article_id = ? AND a.tags_id = b.tags_id')
            ->order('a.tags_id', 'ASC')
            ->bind(array($id))
            ->findAll());
    }

    protected function _getArticleTypeTags() {
        $_r = '<ol>';
        $_tempRs = $this->db->select()
            ->table('`#@__@tags`')
            ->order('tags_id', 'ASC')
            ->findAll();
