...
class Helper {
     ...
     * @return string
     */
    public static function getTreeIMG($level, $typeName, $vp) {
        $_treeShow = '';
        if ($level == 1) {
        ...
        $_treeShow .= '<span title="' . $typeName . '">'
            . \Tools\Html::getLenStr($typeName, 30) . '</span>';
        return $_treeShow;
    }
    ...
    public static function getTopAdminMenu(& $map, $userId, & $power) {
        ...
        if (intval($userId) != 1) {//非超级管理员验证权限
                    ...
                    if (isset($_v1['menu']) && count($_v1['menu']) > 0) {
                        ...
                        foreach ($_v1['menu'] as $_k2 => $_v2) {
                            if (in_array($_k1 . '.' . $_k2, $power, true) &&
                                ...
                                $_menuList .= '<li><a href="javascript:void(0);"'
            ...
            foreach ($map as $_k1 => $_v1) {
                $_menuList .= '<dl id="menu_' . $_k1 . '"><dt rel="' . $_k1 . '/'
                    . key($_v1['menu'])
                    . '"><span class="l"></span><span class="m">'
                    . $_v1['title'] . (isset($_v1['menu']) && count($_v1['menu']) > 0 ? ' ▼' : '')
                    . '</span><span class="r"></span></dt>';
     ...
     */
    public static function getDefaultSearchInfo($aryLanguage, $isShowLanguage = false,
