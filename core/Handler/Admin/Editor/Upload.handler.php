...
namespace Handler\Admin\Editor;
    ...
    public function processRequest(Array & $context) {
        ...
        if (empty($_FILES) === false) {
            ...
            if (@is_uploaded_file($tmpName) === false) {
                $this->editorAlert('临时文件可能不是上传文件。');
            }
            //检查文件大小
            if ($fileSize > $maxSize) {
                $this->editorAlert('上传文件大小超过限制。');
            }
            //检查目录名
            $dirName = empty($_POST['dir']) ? 'image' : trim($_POST['dir']);
            if (empty($extArr[$dirName])) {
                $this->editorAlert('目录名不正确。');
            }
            //获得文件扩展名
