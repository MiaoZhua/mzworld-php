        ...
        foreach ($option as $_k => $_v) {
            switch ($_k) {
                ...
                case 'where' :
                    ...
                    break;
                case 'group' :
                    $option[$_k] = $_v ? ' GROUP BY ' . $_v : '';
                    break;
                case 'having' :
                    $option[$_k] = $_v ? ' HAVING ' . $_v : '';
                    break;
                case 'order' :
                    $option[$_k] = $_v ? ' ORDER BY ' . $_v : '';
                    break;
                case 'limit' :
                    $option[$_k] = $_v && is_array($_v) ? $this->_limit($_v[0], $_v[1]) : null;
//                    $option[$_k] = $_v ? ' LIMIT ' . $_v : '';
    ...
    abstract protected function _limit($start, $end);
    ...
    public function call() {
        if (count($this->_sqlParamStorage) > 0) {
            ...
            return $this->prepareBindLevel2Cache('CALL ' . $this->_sqlParamStorage['procedure'],
                $this->_sqlParamStorage['bind'], $this->_operation);
        }
        return false;
    }
...
//                logger::debug($_match[1]);
    ...
    private function _lazyConn() {
        if (is_null($this->_pdo)) {
            try {
                $_PDOAttr = array(
                    PDO::MYSQL_ATTR_INIT_COMMAND => "SET NAMES '" . $this->_dsn['charset'] . "'",
                    PDO::ERRMODE_EXCEPTION => true,
                    // PDO::ATTR_PERSISTENT => true//使用一个存在的连接，长连接
                );
                ...
                $_dsn = "{$this->_dsn['driver']}:host={$this->_dsn['host']};"
                    ...
                    . "charset={$this->_dsn['charset']};"//本地驱动转义时使用指定的字符集
                    . "port={$this->_dsn['port']}";
