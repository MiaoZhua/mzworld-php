...
class ProxyHandler {
    ...
    private $_clazz = null;
    private $_injectMapping = array(); //自身资源映射
                ...
                if (strcmp($_n, 'value') == 0) {
                    if (is_array($_p)) {
                        foreach ($_p as $_resource) {
                            require_once CORE_PATH . $_resource;
                        }
                    } else {
                        require_once CORE_PATH . $_p;
                    }
                } else {
...
//                require_once CORE_PATH . $value;
     ...
     * @param type $parameName 变量名
     */
    public final function setProxyInject($className, $parameName) {
        //$this->__get($this->_clazz);
        //logger::debug($this->_clazz . '  ' . $className);
        //调用类的变量名可能不一样，如果class name作为key，则会丢失调用相同类但不同变量名的注入
        if (!isset($this->_injectMapping['injects'][$parameName])) {
            $this->_injectMapping['injects'][$parameName] = $className;
        }
    }
    ...
    public final function inject(& $proxyHandlers, & $aryData, & $aryBundles) {
        ...
        if (is_null($this->_instances)) {
            ...
            if (isset($this->_tmpInjectCoordinator['injects']) &&
                ...
                foreach ($this->_tmpInjectCoordinator['injects'] as $_parame => $_class) {
                    if ($proxyHandlers[$_class]->isAopProxy()) {
                        $this->_instances->$_parame = $proxyHandlers[$_class];
                    } else {
                        //PHP5中对象直接为引用，不用传址，否则会产生E_STRICT级别消息
                        $this->_instances->$_parame = $proxyHandlers[$_class]->currentInstances();
                    }
                }
            }
        ...
        if ($this->_hasJoinPoint) {//如果存在切点
            ...
            if ($this->checkJoinPointProperty('inject', $this->_currentMethod)) {
                ...
                if (is_array($_injectValue)) { //__Inject = array('db', '\Service\Helper' => 'helper')
                    foreach ($_injectValue as $_className => $_parameName) {
                        if (is_numeric($_className)) {
                            $_injects = $this->_proxyFactory->getDataRouteConfig();
                            if (isset($_injects[$_parameName])) {
                                $this->_proxyFactory->inject($_injects[$_parameName]);
                                $this->setProxyInject($_injects[$_parameName], $_parameName);
                                $_aryProperty[$_parameName] = $_injects[$_parameName];
                            }
                        } else {
                            $this->_proxyFactory->inject($_className);
                            $this->setProxyInject($_className, $_parameName);
                            $_aryProperty[$_parameName] = $_className;
                        }
                    }
                } else {// __Inject = 'db'
                    $_injects = $this->_proxyFactory->getDataRouteConfig();
                ...
                if (count($_aryProperty) > 0) {
                    foreach ($_aryProperty as $_parame => $_clazz) {
                        $_tmp = $this->_proxyFactory->getProxyHandlers($_clazz);
                        if ($_tmp->isAopProxy()) {
                            $this->_instances->$_parame = $_tmp;
                        } else {
                            //PHP5中对象直接为引用，不用传址，否则会产生E_STRICT级别消息
                            $this->_instances->$_parame = $_tmp->currentInstances();
                        }
                    }
                }
                unset($_aryProperty);
                ...
                switch (key($_v)) {
                    ...
                    case '@injects' :
                        ...
                        if ($_proxyHandlers[$this->_injectMapping['injects'][$_tmpCurrent]]->isAopProxy()) {
                            ...
                            $_args[] = $_proxyHandlers[$this->_injectMapping['injects'][$_tmpCurrent]]->currentInstances();
                        }
