    ...
    public function __construct(& $__ROOT__, & $__DOMAIN__, & $dsn, $db) {
        if (is_null($this->_sessionHandler)) {
            switch (strtolower($dsn['default']['sessionType'])) {
                case 'database':
                    $this->_sessionHandler = new Impl\SessionDatabaseImpl($db);
                    break;
                case 'memcache':
                    $this->_sessionHandler = new Impl\SessionMemcacheImpl($dsn['default']['memServers'],
                        $this->dsn['default']['dbName']);
                    break;
                case 'memcached':
                    $this->_sessionHandler = new Impl\SessionMemcachedImpl($dsn['default']['memServers'],
                        $this->dsn['default']['dbName']);
                    break;
            }
