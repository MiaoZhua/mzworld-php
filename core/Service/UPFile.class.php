...
//                rename($_srcPath, ROOT_PATH . $resource['path'] . $_srcInfo['name']);
    ...
    public function imageMerge(Array $aryImgList, $userId, $fileName) {
        if (count($aryImgList) == 0) {
            return false;
        }
        $_upFolder = 'u' . DIRECTORY_SEPARATOR . $userId;
        File::mkdir($_upFolder, $this->annexFolder);

        $_path = $this->annexFolder . 'pics' . DIRECTORY_SEPARATOR . 'l' . DIRECTORY_SEPARATOR;
        $_src = getimagesize($_path . $aryImgList[0]);
        $_canvas = imagecreatetruecolor($_src[0], $_src[1]);
        $_color   = imagecolorallocate($_canvas, 202, 201, 201);
        imagefill($_canvas, 0, 0, $_color);
        imagecolortransparent($_canvas, $_color);

        foreach ($aryImgList as $_img) {
            $_tmp = imagecreatefrompng($_path . $_img);
            imagesavealpha($_tmp, true);
            imagecopy($_canvas, $_tmp, 0, 0, 0, 0, $_src[0], $_src[1]);
            imagedestroy($_tmp);
        }
        imagepng($_canvas, $this->annexFolder . $_upFolder . DIRECTORY_SEPARATOR . $fileName, 2);
        imagedestroy($_canvas);

        return true;
    }

    public function avatarIntact($userId) {
        $_userFolder = $this->annexFolder . 'u' . DIRECTORY_SEPARATOR . $userId . DIRECTORY_SEPARATOR;
        $_front = $_userFolder . 'avatar_front.png';
        if (is_file($_front)) {
            $_plate = $this->annexFolder . 'avatar_plate.png';
            $_src = getimagesize($_plate);
            $_canvas = imagecreatefrompng($_plate);
            imagesavealpha($_canvas, true);
//            imagealphablending($_canvas, true);

            $_wh = getimagesize($_front);
            $_tmp = imagecreatefrompng($_front);
            imagesavealpha($_tmp, true);
            imagecopymerge($_canvas, $_tmp, ($_src[0] - $_wh[0]) / 2, 0, 0, 0, $_wh[0], $_wh[1], 100);
            imagedestroy($_tmp);

            imagepng($_canvas, $_userFolder . 'avatar_intact.png', 1);
            imagedestroy($_canvas);
        }
        return true;
    }
