...
class AES {
    ...
    public static $key = 'SJlfidshfUDS^&%DFslFSiuJUdf@f({k'; //AES加密密钥 请及时配置，不要与其他项目相同 32位
    public static $iv = '8107186D$y554005'; //AES加密向量，长度尽量不要改变，混淆即可
    ...
    public static final function encrypt($value, $cipherAlg = MCRYPT_RIJNDAEL_128) {
        return bin2hex(mcrypt_encrypt($cipherAlg, self::$key, $value, MCRYPT_MODE_CBC, self::$iv));
    }
    ...
    public static final function decrypt($value, $cipherAlg = MCRYPT_RIJNDAEL_128) {
        return trim(mcrypt_decrypt($cipherAlg, self::$key, pack('H*', $value), MCRYPT_MODE_CBC, self::$iv));
    }
