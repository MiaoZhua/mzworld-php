...
class Othersetting extends CI_Controller {
	
	function Othersetting() {
		session_start();
		parent::__construct ();
		$this->session->set_userdata('menu','othersetting');
		$this->langtype='_en';
		$this->lang->load('gksel','english');
		
		$this->category_id=59;//category_id
		$this->category_key='eDkv5Sf59btGZikJIqGeUhSCG5KTByDv';//category_id
		$this->controller='othersetting';
	}
	...
	function index(){
		...
		$con=array('other_con'=>'category_id in (65,91)','parent'=>$this->category_id,'orderby'=>'category_id','orderby_res'=>'ASC');
		if($keyword!=""){
			$con['keyword']=$keyword;
		}
		//排序--开始
			$actionorderby=doactionorderby($category_info['parameter_list']);
		...
		//排序--结束
		$data['subcategory']=$this->ArticleModel->getarticle_categorylist($con);
		
		//判断category 是否直接进入下一节--开始
			$is_next=doactionisnext($category_info['parameter_list']);
		//判断category 是否直接进入下一节--结束
		if($is_next==1){
			redirect('admins/'.$this->controller.'/article_list?subcategory_ID='.$data['subcategory'][0]['category_id'].'&key='.$data['subcategory'][0]['key']);
		}else{
			$this->load->view('admin/article_category_sub_list',$data);
		}
	...
	function toedit_subcategory(){
		...
		$data['url']='<a href="'.site_url('admins/'.$this->controller.'/index').'">'.$category_info['category_name'.$this->langtype].'</a> > 修改子类别';
		$this->load->view('admin/article_category_sub_edit',$data);
	}
	//修改子分类
