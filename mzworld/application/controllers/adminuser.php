...
//		}
	...
	function index(){
		...
		if ($_GET){
			...
			$url = base_url().'?c=adminuser&m=index&keyword='.$keyword.'&orderby='.$orderby_old.'&orderby_res='.$orderby_res.'&page='.$page;
	...
	function manageadmin($row=0){
		...
		$con=array('orderby'=>'user_id','orderby_res'=>'ASC','row'=>$row,'page'=>$data['page']);
	...
	function toedit_admin($user_id){
		...
		$data['userinfo']=$this->AdminModel->getotheradmininfo($user_id);
	...
	function edit_admin(){
		...
		$this->AdminModel->updateadmin($user_id,$arr);
		redirect('admins/user/manageadmin');
	}
	...
	function edit_user(){
		...
		$this->UserModel->edit_user($user_id,$arr);
		redirect('admins/user');
	}
	 ...
	 * 注册 邮件
	 * */
	function send_audit_email($user_id=0,$langtype='_en'){
		//发送邮件
		$issend=1;
		$str_1='192.168.1';
		$str_2='localhost';
...
//			echo $content;exit;
	...
	function del_user($user_id){
		$this->UserModel->del_user($user_id);
	}
	
	function del_admin($user_id){
		$this->AdminModel->del_admin($user_id);
	}
	...
	function change_user_status($user_id=0,$status=0){
		if($status==1){
			$arr=array('status'=>0);
			$this->UserModel->edit_user($user_id,$arr);	
		}else{
