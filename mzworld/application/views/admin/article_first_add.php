...
?>
<form action="<?php echo site_url('admins/'.$this->controller.'/add_article')?>" method="post">
							...
							<div style="float:left;width:100%;margin:10px 0px 0px 0px;">
								...
								<?php for($la=0;$la<count($langarr);$la++){//多语言?>
									<div style="float:left;margin:3px 0px 0px 5px;">
										ALT ( <?php echo $langarr[$la]['lang_name_en']?> ) <input name="pic_<?php echo $tt.$langarr[$la]['langtype'];?>" value=""/>
									</div>
								<?php }?>
							</div>
						</td>
...
//							echo '<div class="subselectlist_'.$section.'" id="subselectlist_'.$section.'_0" style="float:left;width:100%;text-align:center;padding:5px 0px 5px 0px;height:48px;background:#EFEFEF;" onclick="toselectimg('.$section.',0,\'\')">&nbsp;</div>';
			...
			<?php if($subcategory_info['category_id']==80){?>
				<tr>
				    <td colspan="2" align="left">
				    	<?php 
				    		$data['articleinfo']=array('article_id'=>0);
				    	?>
				    	<?php $this->load->view('admin/product_edit_section_2_color_size',$data)?>
				    </td>
				</tr>
				<tr>
				    <td colspan="2">
				    ...
				    <td align="left">
				    	...
				    	<select name="selection_3">
				    		<?php if(isset($selection_3list)){for($i=0;$i<count($selection_3list);$i++){?>
				    			<option value="<?php echo $selection_3list[$i]['article_id'];?>"><?php echo $selection_3list[$i]['article_name'.$this->langtype]?></option>
				    		<?php }}?>
				    	</select>
				    </td>
				</tr>
				    ...
				    <td align="left">
				    	...
				    	if(isset($checkboxion_1list)){
				    		for($i=0;$i<count($checkboxion_1list);$i++){
				    			$ischecked='';
				    		?>
				    		<div style="float:left;padding:6px 10px 8px 6px;margin:0px 10px 0px 0px;background:#EFEFEF;border-radius:5px;-moz-border-radius:5px;-webkit-border-radius:5px;">
			...
			for($tt=1;$tt<=5;$tt++){if(${'is_nolaninput_'.$tt}==1){?>
				<tr>
				    ...
				    <td align="left">
				    	<div style="float:left;"><input type="text" style="width:<?php echo ${'nolaninput_'.$tt.'_width'}?>px;" name="nolaninput_<?php echo $tt;?>" value="" /></div>
				    </td>
				</tr>
				<tr>
				    <td colspan="2">
					...
					<?php 
						for($la=0;$la<count($langarr);$la++){//多语言
							...
							<tr>
							    ...
							    <td align="left">
							    	<div style="float:left;"><input type="text" style="width:'.${'laninput_'.$tt.'_width'}.'px;" name="laninput_'.$tt.$langarr[$la]['langtype'].'" value=""/></div>
							    </td>
							</tr>
							';
						}
						...
						if(${'lantextarea_'.$tt.'_showtype'}==1){//文本框
							for($la=0;$la<count($langarr);$la++){//多语言
								...
								<tr>
								    ...
								    <td align="left">
								    	<div style="float:left;"><textarea style="float:left;width:'.${'lantextarea_'.$tt.'_width'}.'px;height:'.${'lantextarea_'.$tt.'_height'}.'px;" id="lantextarea_'.$tt.$langarr[$la]['langtype'].'" name="lantextarea_'.$tt.$langarr[$la]['langtype'].'"></textarea></div>
								    </td>
								</tr>
								<tr>
								    <td colspan="2">
