...
?>
<form action="<?php echo site_url('admins/'.$this->controller.'/edit_article')?>" method="post">
<table cellspacing=0 cellpadding=0 width="98%" style="margin-left:1%;color:black;">
	<tr>
		<td>
			<table width="100%">
			<?php 
							...
							<div style="float:left;width:100%;margin:10px 0px 0px 0px;">
								...
								<?php for($la=0;$la<count($langarr);$la++){//多语言?>
									<div style="float:left;margin:3px 0px 0px 5px;">
										ALT ( <?php echo $langarr[$la]['lang_name_en']?> ) <input name="pic_<?php echo $tt.$langarr[$la]['langtype'];?>" value="<?php echo $articleinfo['pic_'.$tt.$langarr[$la]['langtype']]?>"/>
									</div>
								<?php }?>
							</div>
						</td>
			...
			<?php if($is_selection_1==1){?>
				<tr>
				    ...
				    <div class="tabmes_l">
					<?php $selection_1=$articleinfo['selection_1'];?>
...
//							echo '<div class="subselectlist_'.$section.'" id="subselectlist_'.$section.'_0" style="float:left;width:100%;text-align:center;padding:5px 0px 5px 0px;height:48px;background:#EFEFEF;" onclick="toselectimg('.$section.',0,\'\')">&nbsp;</div>';
			...
			<?php }?>
			<?php if($articleinfo['subcategory_id']==80){?>
				<tr>
				    <td colspan="2" align="left">
				    	<?php $this->load->view('admin/product_edit_section_2_color_size')?>
				    </td>
				</tr>
				<tr>
				    <td colspan="2">
			...
			<?php if($is_checkboxion_1==1){?>
				<tr>
					<td width="120" align="right">Tags&nbsp;&nbsp;</td>
				    <td align="left">
				    	<?php 
					    	$postsubcategory_info=$this->ArticleModel->getarticle_categoryinfo(59);
				    	...
				    	if(isset($checkboxion_1list)){
				    		...
				    		?>
				    		<div style="float:left;padding:3px 10px 3px 6px;margin:0px 10px 10px 0px;background:#EFEFEF;border-radius:5px;-moz-border-radius:5px;-webkit-border-radius:5px;">
				...
				<tr>
				    ...
				    <td align="left">
				    	<?php 
							...
							//排序--结束
							$checkboxion_2list=$this->ArticleModel->getarticle_categorylist($con);
				    	?>
				    	<?php 
				    	if(isset($checkboxion_2list)){
				    		for($i=0;$i<count($checkboxion_2list);$i++){
				    			$sql="SELECT * FROM gksel_article_checkboxion WHERE checkboxion_num=2 AND article_id=".$articleinfo['article_id']." AND condition_id=".$checkboxion_2list[$i]['category_id'];
				    			$checkedres=$this->db->query($sql)->row_array();
				    			if(!empty($checkedres)){
				    				$ischecked='checked';
				    			}else{
				    		...
				    		?>
				    		<div style="float:left;padding:3px 10px 3px 6px;margin:0px 10px 10px 0px;background:#EFEFEF;border-radius:5px;-moz-border-radius:5px;-webkit-border-radius:5px;">
				    			<input type="checkbox" name="checkboxion_2[]" id="checkboxion_2_<?php echo $checkboxion_2list[$i]['category_id'];?>" value="<?php echo $checkboxion_2list[$i]['category_id'];?>" <?php echo $ischecked;?> /> <label for="checkboxion_2_<?php echo $checkboxion_2list[$i]['category_id'];?>"><img style="width:12px;" src="<?php echo base_url().$checkboxion_2list[$i]['pic_1']?>"/><?php echo $checkboxion_2list[$i]['category_name'.$this->langtype]?></label>
				    		</div>
				    	<?php }}?>
				    </td>
				</tr>
						...
						if(${'lantextarea_'.$tt.'_showtype'}==1){//文本框
							for($la=0;$la<count($langarr);$la++){//多语言
								...
								<tr>
								    ...
								    <td align="left">
								    	<div style="float:left;"><textarea style="float:left;width:'.${'lantextarea_'.$tt.'_width'}.'px;height:'.${'lantextarea_'.$tt.'_height'}.'px;" id="lantextarea_'.$tt.$langarr[$la]['langtype'].'" name="lantextarea_'.$tt.$langarr[$la]['langtype'].'">'.$articleinfo['lantextarea_'.$tt.$langarr[$la]['langtype']].'</textarea></div>
								    </td>
								</tr>
								<tr>
								    <td colspan="2">
			...
			<?php }}?>
			    <tr>
				    ...
				    <td align="left" style="padding-bottom:10px;">
				    	...
				    	<input name="subcategory_id" type="hidden" value="<?php echo $articleinfo['subcategory_id'];?>" />
