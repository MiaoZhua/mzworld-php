<table cellspacing=0 cellpadding=0 class="tab_post">
	...
	<tr>
		<td colspan="2" onclick="toactionsection(<?php echo $section;?>)" style="font-weight:bold;text-indent:50px;font-size:16px;background:#EFEFEF;padding:5px 0px 5px 0px;">
			<div style="float:left;margin:7px 0px 0px 0px;"><?php echo lang('product_specifications')?></div>
						...
						<div style="float:left;width:630px;background-color: #F8F8F8;border: 1px solid #ECECEC;">
							<div style="float:left;width:98%;margin:10px 0px 10px 2%;">
								<div class="tabmes_l">
									...
									&nbsp;&nbsp;
									<input type="radio" onclick="toselectmuban(2)" name="parameter_muban" id="parameter_muban_2" value="2" checked/> <label for="parameter_muban_2">模板B (有颜色)</label>
									&nbsp;&nbsp;
									<input type="radio" onclick="toselectmuban(3)" name="parameter_muban" id="parameter_muban_3" value="3"/> <label for="parameter_muban_3">模板C (有颜色,大小和其他)</label>
								</div>
							</div>
						</div>
					</div>
						...
						<div style="float:left;width:100%;display:none;">
							<div style="float:left;width:630px;background-color: #F8F8F8;border: 1px solid #ECECEC;margin:0px 0px 10px 0px;">
								<?php 
									...
									$featurelist=$this->ProductModel->getfeaturelist(array('parent'=>$feature_category_id,'section'=>2,'orderby'=>'sort','orderby_res'=>'ASC','row'=>0,'page'=>1));//此处有不一样的地方
									if(!empty($featurelist)){
										$count=count($featurelist);
										$num=4;
										echo '<div style="float:left;width:98%;margin:0px 0px 0px 2%;">';
...
//												}
						...
						<?php 
								...
								$sql="
									...
									WHERE parent=36 ORDER BY feature_id ASC LIMIT 0,1";
...
<!--									<div class="title_size">大小</div>-->
								...
								<div id="product_colorprice_tbody" style="float:left;width:100%;">
									...
									if(!empty($checkedcolorlist)){
										for($i=0;$i<count($checkedcolorlist);$i++){
											...
											if(!empty($checkedsizelist)){
												for($j=0;$j<count($checkedsizelist);$j++){
													$sql="SELECT * FROM gksel_product_compose WHERE product_id=".$productinfo['product_id']." AND color_id=".$checkedcolorlist[$i]['feature_id']." AND size_id=".$checkedsizelist[$j]['feature_id']." AND zdyone_id=".$checkedzdyonelist[0]['feature_id']."";
						...
						<div style="float:left;width:100%;margin:10px 0px 0px 0px;">
							...
							//zdyonecheckbox[]
									...
									echo '<input name="feature_category_name_ch_36" type="hidden" value="'.$zdyonefeaturecategoryinfo['feature_name_ch'].'"/>';
...
//					text_2=text_2+'<div class="list_size list_size_'+checkedsizearr[i]+'">'+$('#sizenameinput_'+checkedsizearr[i]).val()+'</div>';
					...
					text_2=text_2+'<div class="list_merchantcoding"><input id="re_feature_businessnumber_'+color_id+'_'+checkedsizearr[i]+'" name="re_feature_businessnumber_'+color_id+'_'+checkedsizearr[i]+'_'+zdyone_id+'" type="text" style="width:90px" /></div>';
					text_2=text_2+'<div class="list_barcode"><input id="re_feature_eancode_'+color_id+'_'+checkedsizearr[i]+'" name="re_feature_eancode_'+color_id+'_'+checkedsizearr[i]+'_'+zdyone_id+'" type="text" style="width:90px" /></div>';
					text_2=text_2+'</div>';
				}
				text_2=text_2+'</div>';
				//如果是2 则只有颜色，没有大小--所不同的是宽度不一样
				$('#product_colorprice_tbody').append('<div id="product_colorprice_tr_'+color_id+'" style="float:left;width:100%;"><table cellspacing=0 cellpadding=0 style="float:left;"><tr><td width="90" style="border-left: 1px solid #D7D7D7;border-bottom: 1px solid #D7D7D7;">'+text_1+'</td><td width="374">'+text_2+'</td><td width="100" style="border-left: 1px solid #D7D7D7;border-bottom: 1px solid #D7D7D7;border-right: 1px solid #D7D7D7;">'+text_3+'</td></tr></table></div>');
